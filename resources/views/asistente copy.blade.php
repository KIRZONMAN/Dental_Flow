...
<body>
    ...
    <!--TEST-->
    <button id="consultar_citas" type="button">Consultar</button>
    <!--SCRIPT TEST-->
    <script>
        document.getElementById("consultar_citas").addEventListener("click", function () {

            // Realiza la solicitud fetch
            fetch("", {
                method: "POST",
                headers: {
                    "Content-Type": "application/x-www-form-urlencoded" // Asegura que los datos se envíen correctamente
                },
                body: "accion=seleccionar" // Envía un parámetro para identificar la acción
            })
                .then(response => {
                    if (!response.ok) {
                ...
                .then(html => {
                    ...
                    document.getElementById("resultado_consulta_citas").innerHTML = html;
                })
                .catch(error => {
                    // Muestra un mensaje de error en la consola y en el DOM
                    console.error("Error:", error);
                    document.getElementById("resultado_consulta_citas").innerHTML = `<p style="color: red;">Ocurrió un error al consultar las citas. Inténtalo nuevamente.</p>`;
                });
        });
    </script>
